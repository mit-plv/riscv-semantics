...
//      memcpy((void*) &(actual_packet.data), (void*)bytes ,  len);
  ...
  void dump_state() {
   ...
   std::cerr << "   csr mip" << std::hex <<proc.get_state()->mip << std::endl;
...
//	proc.get_state()->mip = 1 << synchronization_packet.cause;
   // }
    ...
    proc.get_state()->mip = synchronization_packet.mip;
    ...
    if(debug) std::cerr << "Instruction match prestep " << proc.get_state()->pc << " " << proc.get_state()->mtvec << "\n" << proc.get_state()->prv << "\n";
 //   if (proc.get_state()->pc + 4 == proc.get_state()->mtvec) hack = true; 
    proc.step(1);
  //  if (hack && (proc.get_state()->pc != proc.get_state()->mtvec)) hack = false; 
//    if (hack==true && proc.get_state()->pc ==  proc.get_state()->mtvec) {hack=false;
    //    actual_packet.pc = proc.get_state()->pc;}


    if(debug) std::cerr << "Instruction match poststep" << proc.get_state()->pc << " "<<proc.get_state()->mtvec <<  " "<< proc.get_state()->mepc  << "\n"<< proc.get_state()->prv << "\n";
    ...
    if(debug) std::cerr << "write 0 in MIP?\n";// << proc.get_state()->pc << " " << proc.get_state()->mtvec << "\n";
// Is this a bit violent?
	...
	if (proc.get_state()->pc == (proc.get_state()->mtvec & ~3)/* && (proc.get_state()->mepc != 0)*/) {
	    if (proc.get_state()->mcause >> 31) {
        if(debug) std::cerr << "Interrupt in spike\n" ;
		actual_packet.interrupt = 1;
		actual_packet.exception = 0;
	    } else {
        if(debug) std::cerr << "Exception in spike\n" ;
		actual_packet.exception = 1;
		actual_packet.interrupt = 0;
	    }
	    actual_packet.cause = proc.get_state()->mcause;
        if(debug) std::cerr << "Exception in spike machine " << actual_packet.cause <<"\n" ;
        ...
        if(debug) std::cerr << "Exception in spike supervisor" << actual_packet.cause <<"\n" ;
	} else {
	    actual_packet.exception = 0;
	}
    }
...
//    }
  ...
  bool comparePackets(VerificationPacket procP,
                      VerificationPacket spikeP) {
    if (spikeP.pc == 0xffffffe0003c5724) dump_state();
    bool match = true;
//    if (((proc.get_state()->mtvec & ~3)==proc.get_state()->pc) &&  procP.pc ==proc.get_state()->pc) return match;
...
//    std::cout << "n" << std::endl;
    do {
      ...
      if (debug) std::cerr << "Got something\n" << command;
    } while (command != "s");
    ...
    std::cin >> mipP;
    if (debug) std::cerr << "mip" << mipP;
    // Adjust interrupt pending
    haskell_packet.mip=mipP;
    ...
    if (haskell_packet.interrupt != 1){ 
            sim.check_packet(haskell_packet);}
    count++;
    if (count% 100000 ==0) std::cout << count<< std::endl;
    if (count >= 23802100) debug=true;
//    if (count >= 13892380) debug=true;
    if (count >= 23802100) sim.dump_state();
