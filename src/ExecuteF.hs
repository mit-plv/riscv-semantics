...
getRoundMode :: (RiscvProgram p t) => MachineInt -> p RoundingMode
getRoundMode rm = do
  frm <- (if rm == 7 then
...
updateFFlags :: (RiscvProgram p t) => ExceptionFlags -> p ()
...
runFPUnary :: (RiscvProgram p t) => (RoundingMode -> Word32 -> F32Result) -> RoundingMode -> Int32 -> p Int32
...
runFPBinary :: (RiscvProgram p t) => (RoundingMode -> Word32 -> Word32 -> F32Result) -> RoundingMode -> Int32 -> Int32 -> p Int32
