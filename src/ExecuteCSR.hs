...
execute (Csrrs rd rs1 csr12) = do
  ...
  val <- loadCSR csr12
  setRegister rd val
  when (rs1 /= 0) $ storeCSR csr12 (val .|. (fromIntegral mask))
...
execute (Csrrsi rd zimm csr12) = do
  val <- loadCSR csr12
  setRegister rd val
  when (zimm /= 0) $ storeCSR csr12 (val .|. (fromIntegral zimm))
execute (Csrrci rd zimm csr12) = do
  val <- loadCSR csr12
  setRegister rd val
  when (zimm /= 0) $ storeCSR csr12 (val .&. (complement $ fromIntegral zimm))
execute _ = mzero
